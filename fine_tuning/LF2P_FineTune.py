...
# trnVol and trnLF files are all .mat
parser = argparse.ArgumentParser()
parser.add_argument('--trnVol-file', type=str, required=True) # 2P 
parser.add_argument('--trnLF-file', type=str, required=True)  # LF time seq 
parser.add_argument('--trnLF2-file', type=str, required=True) # LF time seq
parser.add_argument('--trnLF3-file', type=str, required=True) # LF time seq
parser.add_argument('--trnLF4-file', type=str, required=True) # LF time seq or LF activity map
...
d_optimizer = optim.Adam(D.parameters(), lr=4e-7)#4e-7
...
nLF_Seq=80 # Number of LF frames used
...
file = hdf5storage.loadmat(args.trnLF2_file)
...
file = hdf5storage.loadmat(args.trnLF3_file)
...
lfTrainFTmp4=file['lfTrainTmp'][5::] #If LF activity matfile: which maps are used (in our case last 5 activity maps)
...
lfTrainFTmp4 = lfTrainFTmp4.repeat(nLF_Seq//5, 1, 1, 1)
...
###############################################
            ...
            loss1=2.5*dc_loss+7.5e-4*dc_loss2Seq+0.005*d_loss#attemp 2: loss1=2.5e1*dc_loss+15e-4*dc_loss2Seq+0.001*d_loss
...
###############################################     
    ...
    if (epoch)%1 ==0: 
        torch.save(G.state_dict(),os.path.join(args.outputs_dir, 'epochG_{}.pth'.format(epoch)))
        torch.save(D.state_dict(),os.path.join(args.outputs_dir, 'epochD_{}.pth'.format(epoch)))
